...
export const classifyFreeSupportRecords = (vouchers, now = new Date()) => {
  ...
  const current = [...latest.values()]; const dropped = current.filter((record) => record.status === 'Dropped' || record.dropped === true); const converted = current.filter((record) => record.status === 'Converted' || record.convertedToAmc === true); const available = current.filter((record) => !dropped.includes(record) && !converted.includes(record) && record.status !== 'Renewed')
  const active = available.filter((record) => record.item.amcToDate >= today); const expired = available.filter((record) => record.item.amcToDate < today); const goingToExpire = active.filter((record) => record.item.amcToDate <= warningDate); const newFs = vouchers.filter((voucher) => voucher.freeSupport === 'Yes' && voucher.category === 'New' && voucher.status !== 'Dropped' && voucher.status !== 'Converted')
  return { active, expired, newFs, goingToExpire, dropped, converted }
}
...
export const markFreeSupportConverted = async (id) => { const ref = doc(db, COLLECTION, id); await updateDoc(ref, { status: 'Converted', convertedToAmc: true, updatedAt: serverTimestamp() }); return mapVoucher(await getDoc(ref)) }
...
export const updateFreeSupportVoucher = async (id, data) => { const ref = doc(db, COLLECTION, id); await updateDoc(ref, { voucherDate: data.voucherDate, customerId: data.customerId, customerName: data.customerName, executiveId: data.executiveId, executiveName: data.executiveName, category: data.category, category2: data.category2 || '', narration: (data.narration || '').trim(), freeSupport: data.freeSupport, items: [cleanItem(data.items?.[0])], totalAmount: Number(data.totalAmount), updatedAt: serverTimestamp() }); return mapVoucher(await getDoc(ref)) }
...
export const freeSupportVoucherService = { getNextFreeSupportVoucherNumber, createFreeSupportVoucher, getFreeSupportVouchers, classifyFreeSupportRecords, getFreeSupportDashboardData, markFreeSupportDropped, markFreeSupportConverted, getFreeSupportCustomerCallsReport, updateFreeSupportVoucher, deleteFreeSupportVoucher }
