...
const AmcSummaryReport = ({ reportType }) => {
  ...
  return <div className="page-stack amc-summary-report"><PageHeader title={definition.title} subtitle="AMC records from saved AMC Vouchers." />{error && <div className="auth-error">{error}</div>}{location.state?.message && <div className="auth-success">{location.state.message}</div>}<section className="panel-card report-section">
