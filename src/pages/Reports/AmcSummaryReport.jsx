...
const config = {
  ...
  expired: { title: 'Expired AMC Customers Report', getter: 'getExpiredAMCRecords', status: 'Expired', customerBased: true },
  newAmc: { title: 'New AMC Report', getter: 'getNewAMCRecords', category: 'New' },
  goingToExpire: { title: 'Going to Expire AMC Report', getter: 'getGoingToExpireAMCRecords', status: 'Going to Expire', customerBased: true, warning: true },
...
const AmcSummaryReport = ({ reportType }) => {
  ...
  return <div className="page-stack amc-summary-report"><PageHeader title={definition.title} subtitle="AMC records from saved Sales Vouchers." />{error && <div className="auth-error">{error}</div>}<section className="panel-card report-section">
    ...
    <div className="table-wrap report-table"><table><thead><tr>{definition.warning ? <><th>S.No</th><th>Customer Name</th><th>AMC To Date</th><th>Days Remaining</th><th>Product</th><th>Executive</th><th>Amount</th><th>Status</th></> : definition.customerBased ? <><th>S.No</th><th>Customer Name</th><th>AMC From Date</th><th>AMC To Date</th><th>Product</th><th>Executive</th><th>Amount</th><th>Status</th></> : <><th>S.No</th><th>Voucher No</th><th>Date</th><th>Customer Name</th><th>Product</th><th>Executive</th><th>AMC From</th><th>AMC To</th><th>Amount</th><th>Category</th></>}</tr></thead><tbody>
      {!filtered.length && <tr><td colSpan={definition.customerBased ? 8 : 10} className="text-center">No records found.</td></tr>}
      {paged.map((record, index) => { const item = record.item || getVoucherItem(record); return definition.warning ? <tr key={record.id}><td>{(page - 1) * pageSize + index + 1}</td><td>{emptyReportValue(record.customerName)}</td><td>{formatReportDate(item?.amcToDate)}</td><td>{daysRemaining(record)}</td><td>{emptyReportValue(item?.itemName)}</td><td>{emptyReportValue(record.executiveName)}</td><td>{formatReportCurrency(item?.amount)}</td><td><strong>{definition.status}</strong></td></tr> : definition.customerBased ? <tr key={record.id}><td>{(page - 1) * pageSize + index + 1}</td><td>{emptyReportValue(record.customerName)}</td><td>{formatReportDate(item?.amcFromDate)}</td><td>{formatReportDate(item?.amcToDate)}</td><td>{emptyReportValue(item?.itemName)}</td><td>{emptyReportValue(record.executiveName)}</td><td>{formatReportCurrency(item?.amount)}</td><td><strong>{definition.status}</strong></td></tr> : <tr key={record.id}><td>{(page - 1) * pageSize + index + 1}</td><td>{emptyReportValue(record.voucherNumber)}</td><td>{formatReportDate(record.voucherDate)}</td><td>{emptyReportValue(record.customerName)}</td><td>{emptyReportValue(item?.itemName)}</td><td>{emptyReportValue(record.executiveName)}</td><td>{formatReportDate(item?.amcFromDate)}</td><td>{formatReportDate(item?.amcToDate)}</td><td>{formatReportCurrency(item?.amount)}</td><td><strong>{record.category}</strong></td></tr> })}
