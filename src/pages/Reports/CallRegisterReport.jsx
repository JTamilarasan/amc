    ...
    <section className="panel-card report-section">
      ...
      {loading ? <Loader label="Loading report data..." /> : <>
        ...
        <div className="table-wrap report-table call-register-table"><table><thead><tr>{dayWise ? <><th>Voucher No</th><th>Party Name</th><th>Executive</th><th>Category</th><th>Category 2</th><th>Call Status</th><th>Call Sub Status</th><th>Actions</th></> : <><th>S.No</th><th>Voucher No</th><th>Date</th><th>Party Name</th><th>Executive</th><th>Category</th><th>Category 2</th><th>Call Status</th><th>Call Sub Status</th><th>Next Action</th><th>When</th><th>Actions</th></>}</tr></thead><tbody>
          {!range && <tr><td colSpan={dayWise ? 8 : 12} className="text-center">Select a date range and view type, then generate the report.</td></tr>}
          {range && !filtered.length && <tr><td colSpan={dayWise ? 8 : 12} className="text-center">No records found.</td></tr>}
          {dayWise ? pagedGroups.map((group) => <Fragment key={group.date}><tr className="call-register-day-heading"><td colSpan="8"><strong>{formatReportDate(group.date)}</strong></td></tr>{group.entries.map((voucher) => <tr key={voucher.id}><td>{emptyReportValue(voucher.voucherNumber)}</td><td>{emptyReportValue(voucher.partyName)}</td><td>{emptyReportValue(voucher.executiveName)}</td><td>{emptyReportValue(voucher.category)}</td><td>{emptyReportValue(voucher.category2)}</td><td>{emptyReportValue(voucher.callStatus)}</td><td>{emptyReportValue(voucher.callSubStatus)}</td><td><button type="button" className="executive-action-btn" onClick={() => editVoucher(voucher)}><Pencil size={13} /> Edit</button></td></tr>)}<tr className="call-register-day-total"><td colSpan="8"><strong>Total Entries: {group.entries.length}</strong></td></tr></Fragment>) : pagedVouchers.map((voucher, index) => <tr key={voucher.id}><td>{(page - 1) * pageSize + index + 1}</td><td>{emptyReportValue(voucher.voucherNumber)}</td><td>{formatReportDate(voucher.date)}</td><td>{emptyReportValue(voucher.partyName)}</td><td>{emptyReportValue(voucher.executiveName)}</td><td>{emptyReportValue(voucher.category)}</td><td>{emptyReportValue(voucher.category2)}</td><td>{emptyReportValue(voucher.callStatus)}</td><td>{emptyReportValue(voucher.callSubStatus)}</td><td>{voucher.callStatus === 'Open' ? emptyReportValue(voucher.nextAction) : '-'}</td><td>{voucher.callStatus === 'Open' ? formatReportDate(voucher.when) : '-'}</td><td><button type="button" className="executive-action-btn" onClick={() => editVoucher(voucher)}><Pencil size={13} /> Edit</button></td></tr>)}
        </tbody></table></div>
