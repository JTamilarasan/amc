...
const matchesSearch = (voucher, search) => !search || [voucher.voucherNumber, voucher.partyName, voucher.executiveName, voucher.category, voucher.category2, voucher.callStatus, voucher.callSubStatus]
...
const CallRegisterReport = () => {
  ...
  const generate = async () => {
    ...
    setErrors(next); setLoadError(''); setPage(1)
  ...
  const download = () => {
    ...
    if (range.viewType === 'Day Wise') {
      const rows = groupCallVouchersByDate(records).flatMap((group) => [
        ...
        ...group.entries.map((voucher) => ['', voucher.voucherNumber, voucher.partyName, voucher.executiveName, voucher.category, voucher.category2, voucher.callStatus, voucher.callSubStatus]),
        [`Total Entries: ${group.entries.length}`],
      ])
      exportToCsv({ filename: `call-register-day-wise-${range.from}-to-${range.to}.csv`, headers: ['Date', 'Voucher No', 'Party Name', 'Executive', 'Category', 'Category 2', 'Call Status', 'Call Sub Status'], rows: [...rows, ...summaryRows] })
    } else {
      const rows = sortCallVouchersBySequence(records).map((voucher, index) => [index + 1, voucher.voucherNumber, formatReportDate(voucher.date), voucher.partyName, voucher.executiveName, voucher.category, voucher.category2, voucher.callStatus, voucher.callSubStatus, voucher.nextAction, formatReportDate(voucher.when)])
      exportToCsv({ filename: `call-register-voucher-wise-${range.from}-to-${range.to}.csv`, headers: ['S.No', 'Voucher No', 'Date', 'Party Name', 'Executive', 'Category', 'Category 2', 'Call Status', 'Call Sub Status', 'Next Action', 'When'], rows: [...rows, ...summaryRows] })
    }
  }
  ...
  return <div className="page-stack call-register-report">
    ...
    <section className="panel-card report-section">
      ...
      {loadError && <div className="field-message">{loadError}</div>}
      ...
      {loading ? <Loader label="Loading report data..." /> : <>
        ...
        <div className="table-wrap report-table call-register-table"><table><thead><tr>{dayWise ? <><th>Voucher No</th><th>Party Name</th><th>Executive</th><th>Category</th><th>Category 2</th><th>Call Status</th><th>Call Sub Status</th></> : <><th>S.No</th><th>Voucher No</th><th>Date</th><th>Party Name</th><th>Executive</th><th>Category</th><th>Category 2</th><th>Call Status</th><th>Call Sub Status</th><th>Next Action</th><th>When</th></>}</tr></thead><tbody>
          {!range && <tr><td colSpan={dayWise ? 7 : 11} className="text-center">Select a date range and view type, then generate the report.</td></tr>}
          {range && !filtered.length && <tr><td colSpan={dayWise ? 7 : 11} className="text-center">No records found.</td></tr>}
          {dayWise ? pagedGroups.map((group) => <Fragment key={group.date}><tr className="call-register-day-heading"><td colSpan="7"><strong>{formatReportDate(group.date)}</strong></td></tr>{group.entries.map((voucher) => <tr key={voucher.id}><td>{emptyReportValue(voucher.voucherNumber)}</td><td>{emptyReportValue(voucher.partyName)}</td><td>{emptyReportValue(voucher.executiveName)}</td><td>{emptyReportValue(voucher.category)}</td><td>{emptyReportValue(voucher.category2)}</td><td>{emptyReportValue(voucher.callStatus)}</td><td>{emptyReportValue(voucher.callSubStatus)}</td></tr>)}<tr className="call-register-day-total"><td colSpan="7"><strong>Total Entries: {group.entries.length}</strong></td></tr></Fragment>) : pagedVouchers.map((voucher, index) => <tr key={voucher.id}><td>{(page - 1) * pageSize + index + 1}</td><td>{emptyReportValue(voucher.voucherNumber)}</td><td>{formatReportDate(voucher.date)}</td><td>{emptyReportValue(voucher.partyName)}</td><td>{emptyReportValue(voucher.executiveName)}</td><td>{emptyReportValue(voucher.category)}</td><td>{emptyReportValue(voucher.category2)}</td><td>{emptyReportValue(voucher.callStatus)}</td><td>{emptyReportValue(voucher.callSubStatus)}</td><td>{voucher.callStatus === 'Open' ? emptyReportValue(voucher.nextAction) : '-'}</td><td>{voucher.callStatus === 'Open' ? formatReportDate(voucher.when) : '-'}</td></tr>)}
        </tbody></table></div>
