...
const EnquiryReport = () => {
  ...
  const [fromDate, setFromDate] = useState(returned.fromDate || ''); const [toDate, setToDate] = useState(returned.toDate || '')
  const [followUpLeadId, setFollowUpLeadId] = useState(returned.followUpLeadId || ''); const [range, setRange] = useState({ fromDate: returned.fromDate || '', toDate: returned.toDate || '', followUpLeadId: returned.followUpLeadId || '', disposition: returned.disposition || '' })
  ...
  const leadOptions = useMemo(() => [...new Map(items.filter((item) => item.followUpLeadId).map((item) => [item.followUpLeadId, { id: item.followUpLeadId, name: item.followUpLeadName }])).values()].sort((a, b) => a.name.localeCompare(b.name)), [items])
  ...
  const filtered = useMemo(() => { const search = searchText.trim().toLowerCase(); return generated.filter((item) => !search || [item.companyName, item.customerName, item.contactNumber, item.followUpLeadName].some((value) => String(value || '').toLowerCase().includes(search))) }, [generated, searchText])
  ...
  const download = () => exportToCsv({ filename: 'enquiry-report.csv', headers: ['S.No', 'Company Name', 'Customer Name', 'Contact', 'Next Follow Up', 'Call Disposition', 'Priority', 'Follow Up Lead'], rows: filtered.map((item, index) => [index + 1, item.companyName, item.customerName, item.contactNumber, formatReportDate(item.nextFollowUp), item.callDisposition, item.priority, item.followUpLeadName]) })
  ...
  return <div className="page-stack enquiry-report"><PageHeader title="Enquiry Report" subtitle="View enquiry and follow-up records." />{error && <div className="auth-error">Unable to load enquiry report.</div>}{returned.message && <div className="auth-success">{returned.message}</div>}<section className="panel-card report-section">
    ...
    <div className="toolbar report-toolbar"><div className="search-box"><Search size={16} /><input value={searchText} onChange={(event) => { setSearchText(event.target.value); setPage(1) }} placeholder="Search company, customer, contact or follow-up lead..." /></div><select value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1) }}><option value={10}>10</option><option value={25}>25</option><option value={50}>50</option></select></div>
    <div className="table-wrap report-table enquiry-report-table"><table><thead><tr><th>S.No</th><th>Company Name</th><th>Customer Name</th><th>Contact</th><th>Next Follow Up</th><th>Call Disposition</th><th>Priority</th><th>Follow Up Lead</th><th>Actions</th></tr></thead><tbody>{!filtered.length && <tr><td colSpan="9" className="text-center">No enquiry records found.</td></tr>}{paged.map((item, index) => <tr key={item.id}><td>{(page - 1) * pageSize + index + 1}</td><td>{emptyReportValue(item.companyName)}</td><td>{emptyReportValue(item.customerName)}</td><td>{emptyReportValue(item.contactNumber)}</td><td>{formatReportDate(item.nextFollowUp)}</td><td>{emptyReportValue(item.callDisposition)}</td><td>{emptyReportValue(item.priority)}</td><td>{emptyReportValue(item.followUpLeadName)}</td><td><button type="button" className="executive-action-btn" onClick={() => edit(item)}><Pencil size={13} /> Edit</button></td></tr>)}</tbody></table></div>
