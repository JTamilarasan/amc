...
import { fetchEnquiries, removeEnquiry, selectEnquiryState } from '../../features/enquiries/enquirySlice'
...
const EnquiryReport = () => {
  const { hasPermission } = useAuth(); const canEdit = hasPermission('enquiries', 'edit'); const canDelete = hasPermission('enquiries', 'delete')
  ...
  const edit = (item) => navigate('/enquiry', { state: { editEnquiryId: item.id, returnTo: returned.fromDashboard ? '/dashboard/enquiry' : '/reports/enquiry-report', reportRange: { ...range, followUpLeadName: returned.followUpLeadName } } })
  const deleteEnquiry = async (item) => {
    if (!window.confirm(`Delete enquiry for ${item.customerName || item.contactName || 'this customer'}?`)) return
    try { await dispatch(removeEnquiry(item.id)).unwrap() } catch { /* Redux displays the service error. */ }
  }
