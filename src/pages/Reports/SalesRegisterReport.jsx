...
const SalesRegisterReport = () => {
  ...
  const download = () => exportToCsv({
    ...
    rows: generated.map((voucher) => { const item = getVoucherItem(voucher); return [voucher.voucherNumber, formatReportDate(voucher.voucherDate), voucher.customerName, voucher.executiveName, voucher.category, item?.itemName, item?.serialNo, item?.duration, formatReportDate(item?.amcFromDate), formatReportDate(item?.amcToDate), item?.amount, voucher.status] }),
  })
  ...
  return <div className="page-stack">
    ...
    <section className="panel-card report-section">
      ...
      <div className="table-wrap report-table"><table><thead><tr><th>S.No</th><th>Voucher No</th><th>Date</th><th>Customer Name</th><th>Executive</th><th>Category</th><th>Product</th><th>Serial No</th><th>Duration</th><th>AMC From</th><th>AMC To</th><th>Amount</th><th>Status</th></tr></thead><tbody>
