...
const AreaMaster = () => {
  ...
  return <div className="page-stack">
    ...
    <section className="panel-card"><div className="panel-heading"><h2>Existing Areas</h2></div>
      ...
      {loading && !items.length ? <Loader size="small" label="Loading areas..." /> : isMobile ? <div className="executive-mobile-list">{paged.map((area) => <div className="executive-mobile-card" key={area.id}><div className="executive-mobile-row"><span className="executive-mobile-label">Area</span><span>{area.areaName}</span></div><div className="executive-mobile-row"><span className="executive-mobile-label">Created</span><span>{formatDate(area.createdAt)}</span></div><div className="executive-mobile-actions">{canEdit && <button className="executive-action-btn" onClick={() => { setEditingId(area.id); setAreaName(area.areaName) }}><Pencil size={13} /> Edit</button>}{canDelete && <button className="executive-action-btn delete" onClick={() => { if (window.confirm('Delete this area?')) dispatch(removeArea(area.id)) }}><Trash2 size={13} /> Delete</button>}</div></div>)}</div> :
        <div className="table-wrap"><table><thead><tr><th>S.No</th><th>Area Name</th><th>Created Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>{loading && !items.length && <tr><td colSpan="5"><Loader size="small" label="Loading areas..." /></td></tr>}{!loading && !filtered.length && <tr><td colSpan="5" className="text-center">No areas found.</td></tr>}{paged.map((area, index) => <tr key={area.id}><td>{(page - 1) * pageSize + index + 1}</td><td>{area.areaName}</td><td>{formatDate(area.createdAt)}</td><td><span className="status-badge green">{area.status}</span></td><td><div className="table-actions">{canEdit && <button className="executive-action-btn" onClick={() => { setEditingId(area.id); setAreaName(area.areaName) }}><Pencil size={13} /> Edit</button>}{canDelete && <button className="executive-action-btn delete" onClick={() => { if (window.confirm('Delete this area?')) dispatch(removeArea(area.id)) }}><Trash2 size={13} /> Delete</button>}</div></td></tr>)}</tbody></table></div>}
      <CommonPagination currentPage={page} totalPages={totalPages} totalRecords={filtered.length} onPrevious={() => setPage((value) => Math.max(1, value - 1))} onNext={() => setPage((value) => Math.min(totalPages, value + 1))} className="history-pagination" />
    </section>
  </div>
}
