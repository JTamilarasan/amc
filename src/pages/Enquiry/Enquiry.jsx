...
const Enquiry = () => {
  ...
  return <div className="page-stack"><PageHeader title="Enquiry Voucher" subtitle="Create and manage enquiries and follow-ups." />
    <section className="panel-card form-card"><div className="panel-heading"><h2>{editingId ? 'Edit Enquiry' : 'Enquiry Details'}</h2><span>Lead and follow-up information</span></div><div className="form-grid two-col" style={{ gap: 18 }}>
      <label className="field"><span>Customer Name</span><div className="searchable-select"><input value={form.customerName} onChange={(event) => { const value = event.target.value; const match = customers.find((customer) => customer.customerName.toLowerCase() === value.trim().toLowerCase()); setForm((current) => ({ ...current, customerName: value, customerId: match?.id || '' })); setCustomerOpen(true) }} onFocus={() => setCustomerOpen(true)} onBlur={() => window.setTimeout(() => setCustomerOpen(false), 150)} placeholder="Search and select customer" autoComplete="off" />{customerOpen && <div className="searchable-options">{filteredCustomers.length ? filteredCustomers.map((customer) => <button type="button" key={customer.id} onMouseDown={() => { setForm((current) => ({ ...current, customerId: customer.id, customerName: customer.customerName, contactNumber: customer.mobileNo || '' })); setValidation((current) => ({ ...current, contactNumber: '' })); setCustomerOpen(false) }}>{customer.customerName}</button>) : <div className="searchable-empty">No matching customers</div>}{canAddCustomer && <button type="button" className="searchable-create-option" onMouseDown={() => navigate('/masters/customers', { state: { returnTo: '/enquiry', enquiryReturnSource: 'customer', enquiryForm: form, enquiryEditingId: editingId, enquiryOriginalNavigation: originalNavigation } })}>+ Create Customer</button>}</div>}</div></label>
