...
import { DEFAULT_ENQUIRY_PRODUCTS, ENQUIRY_DISPOSITIONS_BY_PRIORITY, ENQUIRY_DROPPED_REASONS, ENQUIRY_LEAD_SOURCES, ENQUIRY_PRIORITIES } from '../../data/enquiryOptions'
...
const initialForm = (user) => ({ leadCreationDate: dateValue(), contactName: '', contactNumber: '', leadGeneratedBy: '', priority: '', nextFollowUp: '', closedOn: '', remarks: '', customerId: '', customerName: '', areaId: '', areaName: '', receivedExecutiveId: '', receivedExecutiveName: '', productId: '', productName: '', productAmount: '', additionalAmount: '', droppedReason: '', callDisposition: '', assignedExecutiveId: '', assignedExecutiveName: '', followUpLeadId: user?.uid || '', followUpLeadName: user?.displayName || user?.email || '' })
...
const Enquiry = () => {
  ...
  const requiresDroppedReason = form.priority === 'COLD' && form.callDisposition === 'DROPPED'
  const setField = (field, value) => { setForm((current) => { const next = { ...current, [field]: value }; if (field === 'priority') { next.callDisposition = ''; next.nextFollowUp = ''; next.closedOn = ''; next.droppedReason = '' } if (field === 'callDisposition') { if (value !== 'FOLLOWUP') next.nextFollowUp = ''; next.closedOn = value === 'COMPLETED' ? (current.closedOn || dateValue()) : ''; if (value !== 'DROPPED') next.droppedReason = '' } return next }); setValidation((current) => ({ ...current, [field]: '', ...(field === 'priority' || field === 'callDisposition' ? { callDisposition: '', nextFollowUp: '', closedOn: '', droppedReason: '' } : {}) })) }
  ...
  const save = async () => {
    ...
    if (form.productAmount !== '' && (Number.isNaN(Number(form.productAmount)) || Number(form.productAmount) < 0)) next.productAmount = 'Enter a valid Product Amount.'
    if (form.additionalAmount === '' || Number.isNaN(Number(form.additionalAmount)) || Number(form.additionalAmount) < 0) next.additionalAmount = 'Additional Amt is required.'
    if (!form.callDisposition) next.callDisposition = 'Call Disposition is required.'
    if (requiresDroppedReason && !form.droppedReason) next.droppedReason = 'Reason is required when the enquiry is dropped.'
  ...
  return <div className="page-stack"><PageHeader title="Enquiry Voucher" subtitle="Create and manage enquiries and follow-ups." />
    <section className="panel-card form-card"><div className="panel-heading"><h2>{editingId ? 'Edit Enquiry' : 'Enquiry Details'}</h2><span>Lead and follow-up information</span></div><div className="form-grid two-col" style={{ gap: 18 }}>
      ...
      <label className="field"><span>Product Amount</span><input type="number" min="0" step="0.01" value={form.productAmount} onChange={(event) => setField('productAmount', event.target.value)} />{validation.productAmount && <div className="field-message">{validation.productAmount}</div>}</label>
      <label className="field"><span>Additional Amt *</span><input type="number" min="0" step="0.01" value={form.additionalAmount} onChange={(event) => setField('additionalAmount', event.target.value)} />{validation.additionalAmount && <div className="field-message">{validation.additionalAmount}</div>}</label>
      ...
      {requiresDroppedReason && <label className="field"><span>Reason *</span><select value={form.droppedReason} onChange={(event) => setField('droppedReason', event.target.value)}><option value="">Select reason</option>{ENQUIRY_DROPPED_REASONS.map((reason) => <option value={reason} key={reason}>{reason}</option>)}</select>{validation.droppedReason && <div className="field-message">{validation.droppedReason}</div>}</label>}
