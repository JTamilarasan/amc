...
export const removeEnquiry = createAsyncThunk('enquiries/remove', async (id, { rejectWithValue }) => { try { await enquiryService.deleteEnquiry(id); return id } catch (error) { return rejectWithValue(error.message) } })
const slice = createSlice({ name: 'enquiries', initialState, reducers: { clearEnquiryMessage: (state) => { state.error = null; state.successMessage = null } }, extraReducers: (builder) => builder
  ...
  .addCase(removeEnquiry.pending, (state) => { state.loading = true; state.error = null; state.successMessage = null })
  .addCase(removeEnquiry.fulfilled, (state, action) => { state.loading = false; state.items = state.items.filter((item) => item.id !== action.payload); state.successMessage = 'Enquiry deleted successfully.' })
  .addCase(removeEnquiry.rejected, (state, action) => { state.loading = false; state.error = action.payload || 'Failed to delete enquiry.' }) })
