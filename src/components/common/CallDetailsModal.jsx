...
const CallDetailsModal = ({ isOpen, executiveName, filter, vouchers, onClose, onEdit }) => {
  ...
  return <>
    <DetailsModal isOpen={isOpen} title={`${executiveName} - ${label}`} onClose={onClose} size="large">
      ...
      <div className="table-wrap call-details-table executive-call-details-table"><table><thead><tr><th>S.No</th><th>Voucher No</th><th>Date</th><th>Customer Name</th><th>Category</th><th>Category 2</th><th>Call Status</th><th>Call Sub Status</th><th>Next Action</th><th>When</th><th>Actions</th></tr></thead><tbody>
        {!paged.length && <tr><td colSpan="11" className="text-center">No call details found.</td></tr>}
        {paged.map((voucher, index) => <tr key={voucher.id}><td>{(page - 1) * pageSize + index + 1}</td><td><button type="button" className="report-count-link" onClick={() => setViewVoucher(voucher)}>{emptyReportValue(voucher.voucherNumber)}</button></td><td>{formatDate(voucher.date)}</td><td>{emptyReportValue(voucher.partyName)}</td><td>{emptyReportValue(voucher.category)}</td><td>{emptyReportValue(voucher.category2)}</td><td>{emptyReportValue(voucher.callStatus)}</td><td>{emptyReportValue(voucher.callSubStatus)}</td><td>{voucher.callStatus === 'Closed' ? '-' : emptyReportValue(voucher.nextAction)}</td><td>{voucher.callStatus === 'Closed' ? '-' : formatDate(voucher.when)}</td><td><div className="table-actions"><button type="button" className="executive-action-btn" onClick={() => setViewVoucher(voucher)}><Eye size={13} /> View</button><button type="button" className="executive-action-btn" onClick={() => onEdit(voucher)}><Pencil size={13} /> Edit</button></div></td></tr>)}
      </tbody></table></div>
